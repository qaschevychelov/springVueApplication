...
Vue.component('msg-list', {
  ...
  template: '<div style="position: relative; width: 300px;">' +
                '<msg-form :messages="messages" :messageAttr="message" />' +
                '<msg-row v-for="message in messages" ' +
                        ':key="message.id" ' +
                        ':messages="messages" ' +
                        ':message="message" ' +
...
var app = new Vue({
  ...
  template:
    '<div>' +
        ...
        '<div v-else>' +
            '<div><img src="{{profile.userpic}}" /> {{profile.name}}</div>&nbsp;<a href="/logout">Выйти</a>' +
            '<msg-list :messages="messages" />'+
        '</div>' +
    '</div>',
  data: {
